...
router.post("/signup", async (req, res) => {
  try {
    ...
    // Generate JWT
    const token = jwt.sign({ userId: newUser._id }, process.env.JWT_SECRET, { expiresIn: "999999h" });
...
router.post("/login", async (req, res) => {
  ...
  try {
    ...
    // Generate JWT
    const token = jwt.sign({ userId: user._id }, process.env.JWT_SECRET, { expiresIn: "999999h" });
